...
%
% ierr = 1: Structured EigTool doesn't work for 1x1 matrices
% ierr = 2: Structured EigTool doesn't work for sparse matrices
% ierr = 3: (Structured) EigTool doesn't work for m<n rectangular matrices
...
%% Matrix must be 2x2 or larger
  if m==1 & n==1,
      h = errordlg('Sorry, Structured EigTool only works for 2x2 and larger matrices.', 'Error...','modal');      
      waitfor(h);
      ierr = 1;
      return;
  end;
...
%% If the matrix is short and fat, give message
  if m<n,
      h = errordlg('Sorry, Structured EigTool does not work for short, fat rectangular matrices yet.', 'Error...','modal');
      waitfor(h);
      ierr = 3;
      return;
  end;

%% Structured Eigtool currently does not work for sparse matrices
  if issparse(A),
      h = errordlg('Sorry, Structured EigTool does not work for sparse matrices yet.', 'Error...','modal');
      waitfor(h);
      ierr = 2;
      return;
  end;
