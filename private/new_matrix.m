...
% Save the important fields from before
  ...
  if isfield(old_ps_data,'structure'),
    ps_data.structure = old_ps_data.structure;
  end;
...
%% Save the initial values for use when returning to initial plot
  ...
  % Get the structure of the psa
    if isfield(opts,'structure')
        ps_data.structure = opts.structure;
    end
...
%% Pretend the 'Redraw' button has been pressed - we now have our area sorted.
      ps_data = switch_redraw(fig.Number,cax,this_ver,ps_data);
    end
  else
