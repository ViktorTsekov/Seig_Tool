function [Z, x, y, levels, err, T, eigA] = psa_computation(A, T, S, eigA, opts)
...
% a call to fact.m).
%
% A         original matrix (before any Schur decomposition)
% T         input matrix
...
%           structure   structure of the ps
%           unitary_mtx the unitary matrix of the Schur decomposition
%
...
%% shift_axes makes sure that the grid points mirror above and below axis
   ...
   if ( strcmp(opts.structure, 'real') ),
     if ( num_mirror_pts < 0 ) & ( y(end)~=0 ),
        y = [y,0]; 
     elseif ( num_mirror_pts > 0 ) & ( y(1)~=0 ),
        y = [0,y];        
     end
   end
...
if  (strcmp(opts.structure,'Hamiltonian') | strcmp(opts.structure,'skew-symmetric'))
    x = linspace(ax(1),ax(2),x_npts);
    % shift the x-axis to include the 0
...
%% If there is no need to project (all ews are within range)
  if m==np,
    ...
    if opts.no_waitbar~=1,
      if this_ver<6,
        hbar = waitbar(0,'Computing Structured Pseudospectra...');
      else
        hbar = waitbar(0, ['Computing Structured Pseudospectra...(line 1', ...
                                   ' of ',num2str(ly),')']);
      end;
    end;
  else
...
% Update the waitbar
      if opts.no_waitbar~=1,
        if this_ver<6,
          ...
          waitbar(wb_offset+(1-wb_offset)*(ly-j)/ly,hbar,['Computing Structured Pseudospectra...(line ', ...
...
% Set the message and update the window
               the_message = ['Structured EigTool: estimated remaining time is ', ...
...
% Check to see whether we're supposed to be paused; if so, wait
% for resume to be chosen.
        if pause_comp(opts.fig.Number)==1, 
          hdl = findobj(opts.fig,'Tag','Pause');
          waitfor(hdl,'Callback');
        end;
...
%% Actually do the singular value computation
        ...
        switch opts.structure
            case 'unstructured'
                Z(j:-1:last_y,:) = psacore(T,S,q,x,y(j:-1:last_y),1e-5,m-n+1);
            case 'real'
                q = randn(2*n,1); q = q/norm(q);
                Z(j:-1:last_y,:) = psacore_real(A,opts.unitary_mtx,T,q,x,y(j:-1:last_y),1e-15,100)';
            case 'skew-symmetric'
                q = randn(2*n,1); q = q/norm(q);
                Z(j:-1:last_y,:) = psacore_skew(A,opts.unitary_mtx,T,q,x,y(j:-1:last_y),1e-15,100)';
            case 'Hermitian'
                q = randn(n,1); q = q/norm(q);
                Z(j:-1:last_y,:) = psacore_hermitian(A,opts.unitary_mtx,T,q,x,y(j:-1:last_y),1e-15,100)';
            case 'Hamiltonian'
                q = randn(n,1); q = q/norm(q);
                Z(j:-1:last_y,:) = psacore_hamiltonian(A,opts.unitary_mtx,T,q,x,y(j:-1:last_y),1e-15,100)';
            otherwise
                %errordlg([opts.structure ': Unknown structure! This should not ' ...
             %'have happened in the release version.'],'Error...','modal');
             opts.structure
...
if opts.no_waitbar~=1,
  if this_ver<6,
    ...
    waitbar(wb_offset+(1-wb_offset),hbar,['Computing Structured Pseudospectra...(done)']);
  end;
  close(hbar) 
end;
