function Z = psacore_hermitian(A,W,R,q,x,y,tol,maxiter)
%   Z = psacore_hermitian(A,W,R,q,x,y,tol,maxiter)
%
% It computes the HERMITIAN $\mu$ value of $(A - zI)^{-1}$.
% It implements the Bisection method and uses the Lanczos method (with full
% reorthogonalizations) for svd calculations.
%
% A       : Original matrix A
...
for jj = 1:lx
    for kk = 1:ly
        ...
        if n < 10
            B = inv(A - zpt*eye(n));
            Bh = (B-B')/(2*i);
            % Check if Bh is definite
            eigval = eig(Bh);
            if (sum(sign(eigval)==ones(m,1))==m) | (sum(sign(eigval)==-ones(m,1))==m)
%                disp('The matrix B-B^H is definite...')
                Z(jj,kk) = 0;
                definiteflag = 1;
            end
        else
            % Use Lanczos to check quickly if Bh = (B-B')/(2*i) is indefinite
            [V,T,res,k] = lanczsvdBh(W,R,zpt,m,q,tolsmall);
            eigval = eig(T);
            index = find(abs(eigval) > 1e-12);
            lambdamin = min(eigval(index));
            lambdamax = max(eigval(index));
            if sign(lambdamin)*sign(lambdamax) > 0
%                disp('The matrix B-B^H is definite...')
                Z(jj,kk) = 0;
                definiteflag = 1;
                iter = 0;
...
if ~isempty(B)
    ...
    [VV,T,res,mv] = lanczsvdhermitian(W, R, z, gamma, m, q, tol);
...
if ~isempty(B)
    ...
    [VV,T,res,mv0] = lanczsvdhermitian(W, R, z, gamma, m, q, tol);
    mv = mv + mv0;
    % Approximate the second singular value
    Theta = eig(T);
    ...
    [VV,T,res,mvplus] = lanczsvdhermitian(W, R, z, gamma+dh, m, q, tol);
    mv = mv + mvplus;
    % Approximate the second singular value
    Theta = eig(T);
    Theta = real(sqrt(real(Theta)));
    ...
    % Approximate sigma at gamma - dh
    [VV,T,res,mvminus] = lanczsvdhermitian(W, R, z, gamma-dh, m, q, tol);
    mv = mv + mvminus;
    % Approximate the second singular value
    Theta = eig(T);
    Theta = real(sqrt(real(Theta)));
