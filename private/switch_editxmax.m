function ps_data = switch_editxmax(fig,cax,ps_data)

% function ps_data = switch_editxmax(fig,cax,ps_data)
%
% Function called when the max x axis value is changed
...
% This is an unmodified original Eigtool function included in Structured Eigtool.
    ...
    if ~isempty(n) & length(n)==1,
      if n>ps_data.zoom_list{ps_data.zoom_pos}.ax(1),

% If we're zooming out
        if n>ps_data.zoom_list{ps_data.zoom_pos}.ax(2),
...
%% Increment the count
        ...
        ps_data.zoom_list{ps_data.zoom_pos}.ax(2)=str2num(get(gcbo,'String'));
        set(cax,'xlim',ps_data.zoom_list{ps_data.zoom_pos}.ax(1:2));
        set(cax,'ylim',ps_data.zoom_list{ps_data.zoom_pos}.ax(3:4));
      else
        errordlg('Maximum x value must be larger than minimum x value!', ...
                 'Invalid input','modal');
      end;
    else
      errordlg('Invalid number for maximum x value','Invalid input','modal');
    end;
