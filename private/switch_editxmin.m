function ps_data = switch_editxmin(fig,cax,ps_data)

% function ps_data = switch_editxmin(fig,cax,ps_data)
%
% Function called when min x value changed
...
%% Increment the count
        ...
        ps_data.zoom_list{ps_data.zoom_pos}.ax(1)=n;
        set(cax,'xlim',ps_data.zoom_list{ps_data.zoom_pos}.ax(1:2));
        set(cax,'ylim',ps_data.zoom_list{ps_data.zoom_pos}.ax(3:4));
      else
        errordlg('Minimum x value must be smaller than maximum x value!', ...
                 'Invalid input','modal');
      end;
    else
      errordlg('Invalid number for minimum x value','Invalid input','modal');
    end;
