function ps_data = switch_editymax(fig,cax,ps_data)

% function ps_data = switch_editymax(fig,cax,ps_data)
%
% Function called when max y level changed
...
%% Increment the count
        ...
        ps_data.zoom_list{ps_data.zoom_pos}.ax(4)=str2num(get(gcbo,'String'));
        set(cax,'xlim',ps_data.zoom_list{ps_data.zoom_pos}.ax(1:2));
        set(cax,'ylim',ps_data.zoom_list{ps_data.zoom_pos}.ax(3:4));
      else
        errordlg('Maximum y value must be larger than minimum y value!', ...
                 'Invalid input','modal');
      end;
    else
      errordlg('Invalid number for maximum y value','Invalid input','modal');
    end;
