...
%% DK: Since we are always using direct methods, this is always true
...
% EXK: 11/08/2009
    if isfield(ps_data,'structure')
        opts.structure = ps_data.structure;
    else
        opts.structure = 'unstructured';
    end            
    if ps_data.unitary_mtx == 1
        opts.unitary_mtx = eye(n);
    else
        opts.unitary_mtx = ps_data.unitary_mtx;
    end
    ...
    [ps_data.zoom_list{ps_data.zoom_pos}.Z, ...
     ...
     t_levels, ...
     err,output_mtx,output_ews] = psa_computation(ps_data.input_matrix,A,B,the_ews,opts);
